...
module.exports = {
  ...
  init: function(verbose, p2k) {
    ...
    var gen = spawn('bin/'+OSType+'/kindlegen', [name]);
    if(verbose) {
  ...
  restore: function(p2k) {
    ...
    /**
     * remove toc.* in KF8-Demo
     */
    rimraf(path.join(entry, 'toc.*'), function(err, stdout, stderr) {
      if (err) {
        log.err(err);
      } else {
        log.info('remove toc.* in ' + entry);
      }
    });
    
    /**
     * remove *.opf in KF8-Demo
     */
    rimraf(path.join(entry, '*.opf'), function(err, stdout, stderr) {
      if (err) {
        log.err(err);
      } else {
        log.info('remove *.opf in ' + entry);
      }
    });
